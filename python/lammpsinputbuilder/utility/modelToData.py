...
def extractElementsFromData(data_path: str) -> str:
    """
...
def molecule_to_lammps_input(
    ...
    with open(lammps_script_file_path, "w") as f:
        ...
        elements = extractElementsFromData(data_file_path)
        ...
        if ffType == Forcefield.REAX:
            ...
            script_content += f'pair_coeff     * * {forcefieldName}{elements}\n'
            if electrostaticMethod == ElectrostaticMethod.ACKS2:
                script_content += 'fix            ReaxFFSpec all acks2/reaxff 1 0.0 10.0 1e-8 reaxff\n'
            elif electrostaticMethod == ElectrostaticMethod.QEQ:
                script_content += 'fix            ReaxFFSpec all qeq/reaxff 1 0.0 10.0 1e-8 reaxff\n'
            else:
                raise NotImplementedError(
                    f"Electrostatic method {electrostaticMethod} not supported.")
        ...
        script_content += f"neighbor       {min([2.5, minCellDim/2])} bin\n"
